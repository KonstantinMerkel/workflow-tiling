...
export class ScreenEstate {
    constructor(pct_x, pct_y, pct_w, pct_h) {
        if (pct_x < 0 || pct_y < 0 || (pct_x + pct_w) > 100.01 || (pct_y + pct_h) > 100.01) {
    ...
    toAbsolute(monitorRect) {
        return {
            x: Math.round(monitorRect.x + (monitorRect.width * (this.pct_x / 100))),
            y: Math.round(monitorRect.y + (monitorRect.height * (this.pct_y / 100))),
            width: Math.round(monitorRect.width * (this.pct_w / 100)),
            height: Math.round(monitorRect.height * (this.pct_h / 100))
        };
    }
}
