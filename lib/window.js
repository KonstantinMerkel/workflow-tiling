...
export class WindowWrapper {
    ...
    // This uses a one-shot signal connection to prevent recursive resizing loops.
    bindSizeChanged() {
        if (this._sizeChangedHandled) return;
        if (!this.signals.has('size-changed')) {
            this.signals.set('size-changed', this.window.connect('size-changed', () => {
                this._sizeChangedHandled = true;
    ...
    disconnectSignal(name) {
        try {
            ...
            if (id && this.window && this.window.handler_is_connected(id)) {
