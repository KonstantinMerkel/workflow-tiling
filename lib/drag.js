...
export class DragManager {
    ...
    startDragTracking(window) {
        ...
        let originalSlot = layout.getWindowSlot(wrapper.monitorId, window);
    ...
    endDragTracking(window) {
        ...
        if (lastHoveredMonitorId && lastHoveredMonitorId !== wrapper.monitorId) {
            ...
            if (pointerMonitorId && pointerMonitorId !== wrapper.monitorId) {
                ...
                this._commitSameMonitorDrop(window, wrapper, layout, x, y, monitorRect, gaps, origRect, activeDrag.originalSlot, targetSlot);
            }
        }
    }
    ...
    _commitSameMonitorDrop(window, wrapper, layout, x, y, monitorRect, gaps, origRect, originalSlot, targetSlot) {
        let swapped = false;
        ...
        if (originalSlot === -1) {
            if (targetSlot !== -1) {
                ...
                if (behavior === 'swap' && targetWindowCount > 0) {
                    const targetWindows = layout.getWindowsForMonitor(wrapper.monitorId);
                    const targetWindow = targetWindows.find(w => layout.getWindowSlot(wrapper.monitorId, w) === targetSlot);
                    if (targetWindow) {
                        layout.replaceWindow(targetWindow, window, wrapper.monitorId);
                        swapped = true;
                    }
                } else if (targetWindowCount < layout.escalator.getMaxCount()) {
                    layout.trackWindow(window, wrapper.monitorId, targetSlot);
                    swapped = true;
                }
            }
        } else {
            swapped = layout.swapWindowByPointer(wrapper.monitorId, window, x, y, monitorRect, gaps);
        }
