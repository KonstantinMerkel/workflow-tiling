    ...
    _findClosestBoundaryWindow(targetTracker, direction, sourceRect) {
        ...
        for (const win of targetTracker.windows) {
            ...
            if (direction === 'left' || direction === 'right') {
                overlap = Math.min(sourceRect.y + sourceRect.height, targetRect.y + targetRect.height) - Math.max(sourceRect.y, targetRect.y);
            } else if (direction === 'up' || direction === 'down') {
                overlap = Math.min(sourceRect.x + sourceRect.width, targetRect.x + targetRect.width) - Math.max(sourceRect.x, targetRect.x);
            }
        ...
        candidates.sort((a, b) => {
            if (Math.abs(b.overlap - a.overlap) > 0.001) {
    ...
    moveWindowDirection(monitorId, window, direction) {
        ...
        if (this.controller && this.controller.monitorManager) {
            ...
            if (adjacentMonitorIndex !== -1) {
                ...
                if (this.controller._windowWrappers) {
