/**
 * TilingConfig: Singleton for global tiling parameters.
 */
export const TilingConfig = Object.freeze({
    GAPS: {
        INNER: 6,
        OUTER: 4
    }
...
export class ScreenEstate {
     ...
     * Detects inner vs outer edges based on percentage boundaries.
     */
    toAbsolute(monitorRect) {
        const base = {
        ...
        const innerHalf = TilingConfig.GAPS.INNER / 2;
        const outer = TilingConfig.GAPS.OUTER;
        ...
        return {
            x: Math.round(base.x + gaps.left),
            y: Math.round(base.y + gaps.top),
            width: Math.round(base.w - gaps.left - gaps.right),
            height: Math.round(base.h - gaps.top - gaps.bottom)
        };
    }
}
