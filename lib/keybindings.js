...
export class KeybindingManager {
    ...
    _buildDefinitions() {
        ...
        // Batch Utilities
        const utilities = {
            'shortcut-close-monitor': (c, win) => c.closeMonitorWindows(global.display.get_current_monitor(), c.settings.settings.get_boolean('close-monitor-include-minimized')),
            'shortcut-close-workspace': (c, win) => c.closeWorkspaceWindows(global.workspace_manager.get_active_workspace()),
            'shortcut-switch-monitor': (c, win) => c.switchMonitors(win ? win.get_monitor() : global.display.get_current_monitor()),
            'shortcut-port-monitor-left': (c, win) => c.portMonitorToWorkspace(global.display.get_current_monitor(), 'left'),
            'shortcut-port-monitor-right': (c, win) => c.portMonitorToWorkspace(global.display.get_current_monitor(), 'right'),
            'shortcut-unminimize-workspace': (c, win) => c.unminimizeWorkspace(global.workspace_manager.get_active_workspace())
        };
    ...
    bindAll() {
        ...
        for (const def of this._definitions) {
            const { active, keyToBind } = this._resolveBinding(def);
            if (!active) {
                Logger.debug(`Binding ${def.defaultKey} is inactive.`);
                continue;
            }

            Logger.debug(`Resolved binding for ${def.defaultKey}: keyToBind=${keyToBind}, conflict=${def.conflict}`);
