...
export class KeybindingManager {
    constructor(controller) {
        ...
        const protectedKeys = this._definitions
            .filter(d => d.isNativeHijack)
            .map(d => d.conflict);
        this._shadowManager = new ShadowManager(this.settings, protectedKeys);
    }
    ...
    _buildDefinitions() {
        ...
        const nativeHijacks = {
            'maximize': (c) => c.toggleOverrideActiveWindow('maximize'),
            'unmaximize': (c) => c.toggleOverrideActiveWindow('maximize'),
            'toggle-fullscreen': (c) => c.toggleOverrideActiveWindow('fullscreen')
        };

        for (const [key, action] of Object.entries(nativeHijacks)) {
            defs.push({
                isNativeHijack: true,
                conflict: key,
                action: (c, win) => {
                    Logger.debug(`Action triggered: native hijack ${key}`);
                    action(c, win);
                }
            });
        }
    ...
    bindAll() {
        ...
        for (const def of this._definitions) {
            if (def.isNativeHijack) continue;
