    ...
    tilingRequest(window) {
        ...
        try {
            ...
            if (isMonitorChange) {
                ...
                const sourceSlot = sourceTracker.getSlot(window) !== undefined ? sourceTracker.getSlot(window) : 0;
                ...
                let enteringEdge = 'left';
                if (Math.abs(dx) >= Math.abs(dy)) {
                    enteringEdge = dx > 0 ? 'left' : 'right';
                } else {
                    enteringEdge = dy > 0 ? 'top' : 'bottom';
                }

                layout.handleMonitorTransition(window, sourceMonitorId, monitorId, enteringEdge, sourceSlot);
