...
export class MonitorManager {
    ...
    getMonitorInDirection(currentMonitorIndex, direction) {
        try {
            ...
            if (!sourceMonitor) {
                Logger.info(`getMonitorInDirection: invalid sourceMonitor for index ${currentMonitorIndex}`);
                return -1;
            }

            const sRect = global.display.get_monitor_geometry(currentMonitorIndex);
            Logger.info(`[DEBUG] getMonitorInDirection: source monitor ${currentMonitorIndex} rect: ${sRect.x}, ${sRect.y}, ${sRect.width}, ${sRect.height}`);
            ...
            for (let i = 0; i < logicalMonitors.length; i++) {
                ...
                Logger.info(`getMonitorInDirection: checking monitor ${i} (${cRect.x},${cRect.y},${cRect.width},${cRect.height}) for direction ${direction}: inDirection=${inDirection}`);
            ...
            if (candidates.length === 0) {
                Logger.info('getMonitorInDirection: no candidates found');
                return -1;
            }
            ...
            Logger.info(`getMonitorInDirection: best candidate is ${candidates[0].index}`);
