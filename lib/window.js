...
export class WindowWrapper {
    ...
    bindSizeChanged() {
        ...
        if (!this.signals.has('position-changed')) {
            this.signals.set('position-changed', this.window.connect('position-changed', () => {
                if (this._isResizing) return;
                this.controller.tilingRequest(this.window);
            }));
        }
    ...
    applyGeometry(rect) {
        if (!isTilable(this.window) || !this.window.move_resize_frame || this.window.minimized) return;
        ...
        try {
            ...
            if (this.window.maximized_horizontally || this.window.maximized_vertically) {
                ...
                const laterId = global.compositor.get_laters().add(Meta.LaterType.BEFORE_REDRAW, () => {
                    if (!isTilable(this.window)) return false;
                    this._doResize(rect);
                    return false;
                });
